#!/usr/bin/env python3
"""
BTC Precision Scanner — 1h / 4h
Enhanced Version: Aligned with XAG high-precision logic.
Features: Dynamic ATR risk, Session-aware filtering, Wick Trap detection, 
and HTF (4h+1D) trend bias.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='BTC Precision Scanner — Optimized for 1h/4h Trading',
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser.add_argument('-tf', '--timeframe', type=str, default=None,
        help='Single timeframe: 1h or 4h. Omit to scan both.')
...
def fetch_order_book_imbalance(symbol, depth_pct=0.005):
    """BTC specific: wider depth for higher timeframe liquidity check"""
    try:
        ex = get_exchange()
...
def get_btc_trend_bias():
    """Uses 4h and 1D for structural context on 1h/4h signals"""
    total_score = 0.0
    results = {}

    for tf, weight in [('4h', 0.4), ('1d', 0.6)]:
...
TF_PARAMS = {
    ...
    '4h': {
        ...
        'rsi_period': 14,
        'ma_period': 100,
        'atr_stop_mult': 2.0,  # Wider stop for 4h
        'atr_tp_mults': [1.5, 3.0, 5.0]
    }
}
...
def detect_btc_signals(df, tf, bias, use_bias=True):
    ...
    if candle_range > 0:
        ...
        if high[-1] > upper and current < upper:
            ...
            if upper_wick > (candle_range * 0.5):
                candidates.append({
                    'dir': 'SELL', 'type': 'BTC_LIQUIDITY_SWEEP', 'conf': 75, 
        ...
        if low[-1] < lower and current > lower:
            ...
            if lower_wick > (candle_range * 0.5):
                candidates.append({
                    'dir': 'BUY', 'type': 'BTC_LIQUIDITY_SWEEP', 'conf': 75, 
    ...
    if not candidates:
        if current > lower and prev <= lower and rsi_val < 48:
            ...
            candidates.append({'dir': 'BUY', 'type': 'BTC_BANDS_BUY', 'conf': conf, 'desc': f'BTC recovery inside lower band (RSI {rsi_val:.0f})'})

        if current < upper and prev >= upper and rsi_val > 52:
            conf = min(80, (rsi_val - 52) * 1.8 + 50)
            candidates.append({'dir': 'SELL', 'type': 'BTC_BANDS_SELL', 'conf': conf, 'desc': f'BTC rejection from upper band (RSI {rsi_val:.0f})'})
    ...
    for sig in candidates:
        ...
        if session == 'ASIAN_QUIET':
            c -= 10 # BTC is global, less penalty than Silver
            notes.append("⚠️ Quiet Asian session - Reduced volatility")
        elif session == 'LONDON_NY_OVERLAP':
            c += 15
            notes.append("🔥 High liquidity session (London-NY Overlap)")

        if is_dead_market:
            c -= 20
            notes.append("⚠️ Volatility Compressed (ATR Filter activated)")
        ...
        if sig['dir'] == 'BUY':
            if ob_imbalance >= 1.4:
                ...
                c -= 20
                notes.append(f"⚠️ Heavy Ask Walls: {ob_imbalance:.2f}x ratio")
        else:
            if ob_imbalance <= 0.6:
                c += 15
                ...
                c -= 20
                notes.append(f"⚠️ Heavy Bid Walls: {ob_imbalance:.2f}x ratio")
...
def print_signal(sig, plan, tf, verbose=False):
    ...
    print(f"\n  {emoji} [{tf}] BTC/USDT — {sig['dir']}  |  Conf: {sig['conf']}%")
    print(f"     Type: {sig['type']} | {sig['desc']}")
    print(f"     Price: ${sig['price']:,.2f} | RSI: {sig['rsi']} | ATR: {sig['atr']:.2f}")
    print(f"     📍 Entry: ${plan['entry']:,.2f} | 🛑 Stop: ${plan['stop']:,.2f} (${plan['risk_$']:.2f} risk)")
    print(f"     🎯 TP1: ${plan['tp1']:,.2f} | TP2: ${plan['tp2']:,.2f} | TP3: ${plan['tp3']:,.2f}")
    print(f"     📊 Size: {plan['units']} BTC (${plan['value']:,.2f} value)")
    if verbose:
        print("     Audit Trail:")
        for n in sig['notes']:
            print(f"       {n}")
...
def run_scan(timeframes, account_size, risk_pct, use_bias, verbose, min_conf):
    print(f"\n{'='*70}")
    print(f"  📡 BTC PRECISION ENGINE — {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}")
    print(f"{'='*70}")

    bias, score, bias_details = get_btc_trend_bias()
    icon = '🟢' if 'BULLISH' in bias else ('🔴' if 'BEARISH' in bias else '⚪')
    print(f"  {icon}  HTF BIAS (4h+1D): {bias} (Score: {score:+.0f}) | Session: {get_trading_session()}")
    print(f"{'='*70}")

    for tf in timeframes:
        print(f"  ⏱️  Scanning [{tf}]...")
        df = fetch_ohlcv('BTC/USDT', tf, limit=TF_PARAMS[tf]['lookback'] + 20)
        if df is None:
            continue
...
def main():
    ...
    if args.timeframe:
        if args.timeframe not in ('1h', '4h'):
            print("❌ System error: Valid timeframes for this version are 1h or 4h.")
            sys.exit(1)
