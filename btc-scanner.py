...
Usage:
    python btc_scanner.py                  # Scan both 15m and 30m
    python btc_scanner.py -tf 15m          # 15m only
    python btc_scanner.py -tf 30m          # 30m only
    python btc_scanner.py --loop 5         # Repeat every 5 minutes
    python btc_scanner.py --both           # Force dual-timeframe scan (default)
    python btc_scanner.py --no-bias        # Disable trend-bias filter (all signals equal)
    python btc_scanner.py --account 5000   # Set account size
    python btc_scanner.py --risk 0.01      # Set risk % per trade (default 1%)
"""

import ccxt
import pandas as np_pd_alias  # just for type hints — real import below
import pandas as pd
import numpy as np
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='BTC Intraday Scanner — 15m/30m with trend bias',
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser.add_argument('-tf', '--timeframe', type=str, default=None,
        help='Single timeframe: 15m or 30m. Omit to scan both.')
    parser.add_argument('--both', action='store_true',
        help='Explicitly scan both 15m and 30m (default when -tf not set)')
...
def squeeze_momentum(high, low, close, bb_period=20, bb_mult=2.0, kc_mult=1.5):
    """
    Squeeze: BB inside KC → coiling, breakout imminent.
    Returns (squeeze_on, squeeze_release, momentum_bullish)
    """
...
def hurst_exponent(price, window=80):
    """H < 0.45 = mean-reverting, H > 0.55 = trending"""
    if len(price) < window:
        return 0.5
...
def detect_market_structure(high, low, swing=10):
    """Returns 'bullish', 'bearish', or 'neutral' based on swing HH/HL or LH/LL"""
    if len(high) < swing * 3:
        return 'neutral'
    sh, sl = [], []
...
def find_ob_and_fvg(high, low, close, lookback=30):
    """
    Order block: last opposing candle before a 2x-avg impulse.
    FVG: 3-bar gap.
    Returns ob_zone (low, high, dir), fvg_zone (low, high, dir) or Nones.
    """
    ...
    for i in range(len(close)-1, max(0, len(close)-lookback), -1):
        try:
            ...
            if move > avg_move * 1.8:
                if close[i] > close[i-1]:   # bullish impulse → bullish OB
                    ob_zone = (float(low[i-1]), float(high[i-1]), 'bullish')
                else:                         # bearish impulse → bearish OB
    ...
    for i in range(len(high)-3, max(0, len(high)-lookback), -1):
        try:
            if high[i] < low[i+2]:   # bullish FVG
                fvg_zone = (float(high[i]), float(low[i+2]), 'bullish')
                break
            elif low[i] > high[i+2]: # bearish FVG
...
def get_btc_trend_bias():
    ...
    for tf, weight in [('1h', 0.4), ('4h', 0.6)]:
        ...
        # RSI
        rsi_val = rsi(c[-30:], 14)
        rsi_score = (rsi_val - 50) * 2   # -100..+100

        # Price vs EMA50
        ema50 = ema(c, 50)
        ema_score = ((c[-1] - ema50) / ema50) * 1000  # normalized
...
# ============================================
# TIMEFRAME PARAMS (15m & 30m TUNED)
# ============================================

TF_PARAMS = {
    '15m': {
        'lookback': 150,
        'bandwidth': 3.0,      # tighter envelope for scalps
        'multiplier': 1.8,     # narrower bands → more signals
        'rsi_period': 8,       # faster RSI
        'ma_period': 50,
        'confirm_tf': '1h',
        'stop_pct': 0.008,     # 0.8% stop (tight for 15m BTC)
        'tp1_pct': 0.008,
        'tp2_pct': 0.015,
        'tp3_pct': 0.025,
    },
    '30m': {
        ...
        'ma_period': 100,
        'confirm_tf': '1h',
        'stop_pct': 0.012,     # 1.2% stop for 30m
        'tp1_pct': 0.012,
        'tp2_pct': 0.022,
        'tp3_pct': 0.035,
    }
...
def detect_btc_signals(df, tf, bias, use_bias=True):
    """
    Core signal engine for BTC intraday.
    Returns list of signal dicts, sorted by confidence desc.
    bias:  the macro bias string from get_btc_trend_bias()
    """
    p = TF_PARAMS[tf]
    ...
    for sig in candidates:
        ...
        if use_bias:
            if bias in ('BEARISH_STRONG', 'BEARISH'):
                if sig['dir'] == 'SELL':
                    ...
                    # Counter-trend buy: only allow if very oversold
                    if rsi_val < 20:
                        c -= 10
        ...
        # ── HURST / TREND QUALITY ─────────────────────────────────────
        # For intraday BTC: trending (H>0.55) is better for momentum trades
        if h_exp > 0.55:
            c += 8
        ...
        if ob_low is not None:
            ...
            if in_ob and ob_dir == sig['dir'].lower() + 'ish':  # 'bullish'/'bearish'
                c += 10
...
def print_signal(sig, plan, tf, verbose=False):
    ...
    dir_pad = sig['dir']
    print(f"\n  {emoji} [{tf}] BTC/USDT — {dir_pad}  |  Conf: {sig['conf']}%  |  {sig['desc']}")
...
def main():
    ...
    if args.timeframe:
        if args.timeframe not in ('15m', '30m'):
            print(f"❌ This scanner supports 15m and 30m only. Got: {args.timeframe}")
            sys.exit(1)
        timeframes = [args.timeframe]
    else:
        timeframes = ['15m', '30m']   # default: both
